...
// دالة تحويل البيانات لصيغة Base64 المتوافقة مع الروابط
function b64u(s){
...
// دالة الحصول على Access Token من جوجل
async function getT(){
...
async function editEmp(gt,rowNum,name,job){
  await su(gt,"Employees!A"+rowNum+":B"+rowNum,[[name,job]]);
  return {success:true};
}

async function delEmp(gt,rowNum){
  await sc(gt,"Employees!A"+rowNum+":D"+rowNum);
  return {success:true};
}
...
async function curPresent(gt){
  ...
  for(var k=0; k<keys.length; k++){
    if(last[keys[k]][3] && !last[keys[k]][4]) cnt++;
  }
  return cnt;
}
...
async function regAtt(gt,qrCode,mode,scannedBy){
  ...
  for(var j=1; j<arows.length; j++){
    if(arows[j][0]===date && arows[j][1]===emp.id){
      li = j+1; lr = arows[j]; cnt++;
    }
  }
  if(mode==="in"){
...
exports.handler = async function(event){
  ...
  var p = {};
  try{
    p = event.httpMethod==="POST" ? JSON.parse(event.body||"{}") : event.queryStringParameters||{};
  }catch(e){
