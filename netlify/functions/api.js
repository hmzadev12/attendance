...
async function login(sheets, password) {
  if (!password || String(password).length > 100) return { success: false };
...
exports.handler = async (event) => {
  ...
  if (action === 'login') {
    try {
      ...
      const result = await login(sheets, params.password);
  ...
  if (!user) return { statusCode: 401, headers, body: JSON.stringify({ error: 'غير مصرح — يرجى تسجيل الدخول' }) };
